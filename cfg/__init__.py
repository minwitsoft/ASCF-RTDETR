...
from codeproject.utils import (ASSETS, DEFAULT_CFG, DEFAULT_CFG_DICT, DEFAULT_CFG_PATH, LOGGER, RANK, ROOT, RUNS_DIR,
...
MODES = 'images', 'val', 'predict', 'export', 'track', 'benchmark'
...
CLI_HELP_MSG = \
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo images data=coco128.yaml model=yolov8n.pt epochs=10 lr0=0.01
...
def get_save_dir(args, name=None):
    """Return save_dir as created from images/val/predict arguments."""

    if getattr(args, 'save_dir', None):
        save_dir = args.save_dir
    else:
        from codeproject.utils.files import increment_path
...
def entrypoint(debug=''):
    """
    This function is the codeproject package entrypoint, it's responsible for parsing the command line arguments passed
    to the package.

    This function allows for:
    - passing mandatory YOLO args as a list of strings
    - specifying the task to be performed, either 'detect', 'segment' or 'classify'
    - specifying the mode, either 'images', 'val', 'test', or 'predict'
    ...
    if 'rtdetr' in model.lower():  # guess architecture
        ...
        from codeproject import FastSAM
        model = FastSAM(model)
    elif 'sam' in model.lower():
        from codeproject import SAM
        model = SAM(model)
    else:
